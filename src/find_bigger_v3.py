...
def normalize_num(num):
    ...
    for bit in num:
        if bit_index == 0 and cmp(bit, '-') == 0:
            ...
            ret_num += bit
        bit_index += 1
    if cmp(ret_num, "") == 0 or cmp(ret_num,'-') == 0:
        ret_num = '0'
    return 0, ret_num   
...
def find_neg_num(neg_num):
    ...
    for bit in neg_num[::-1][2:]:
        if float(low_bits_sum) / (len(neg_num) - 1 - bit_idx) > 9 or neg_num[bit_idx] == '0': 
            low_bits_sum += ord(neg_num[bit_idx]) - ord('0')
            bit_idx -= 1
            continue
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2 or len(sys.argv[1]) >= 100:
        print "error: input number's length is biger than 100 or the number of input args is not 1."
        sys.exit(1)
