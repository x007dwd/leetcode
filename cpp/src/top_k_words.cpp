...
std::string specail_chara = ",./<>?;':\"[]\\{}|`1234567890-=~!@#$%^&*()_+";
...
void printKMostFreq(FILE *fp, int k) {
    ...
    while (fscanf(fp, "%s", buffer) != EOF) {
        ...
        while (*ptr) {
            ...
            if (specail_chara.find(*ptr) != std::string::npos) {
                chara[0] = *ptr;
                chara[1] = 0;
                *ptr = 0;
                insertTrieAndHeap(chara, &root, minHeap);
            }
            ptr++;
        }
        if (buffer[0] != 0) {
...
int main() {
    string filename;
    cout << "the name of the file " << endl;
    cin >> filename;
    cout << "an integer k." << endl;
